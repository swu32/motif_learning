...
var train_data,test_data,block_structure, condition_structure,condition;
var inst; // instruction sequence. 
block_structure = ['train', 'test'];
...
var gender;  
var bonus_max = 4;
maxtrialbonus = bonus_max/(n_train_trial + n_test_trial);

n_block = block_structure.length
var overallbonus = 0;
var reward = 0;
base_pay = 4;
...
demo_trial = ['J','K','D','S', 'E', 'K','J','S','L','E','J','K','L','K'];
demo_trial_compact = ['J','K','D','S','K','J','S','L','J','K','L','K'];
var trialrecalldemo = [];
var demo_counter = -1;
...
teststructure = permute(['m1']);
	return teststructure;
}

function assigncondition(){
condition = ['ind','m1'][randomNum(0,1)];
...
function assigndata(condition, teststructure){
	...
	train_data =[];
	condition_structure = [];
	idx = randomNum(0, m1_train.length-1);
	...
	for(var i = 0; i < teststructure.length; i++){
	// assign the data of the train block and the test blocks 
...
function nextblock() {
	...
	if (blockcounter == n_block){// end of the third block 
		finishexperiment();}
	else{
